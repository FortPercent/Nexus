...
def _project_scope_filter(query, DocumentModel, project_id: str, user_id: str):
    """TeleAI patch 2026-04-20: # 下拉跨项目隔离.
    当 filter 里带 project_id 时, knowledge 只返:
      - scope=org (组织库, 永远可见)
      - scope=personal 且 meta owner 匹配 user_id (个人库只给自己)
      - scope=project 且 project_slug == project_id (本项目知识)
    其他 scope 或缺 meta 的条目一律隐藏, 防止 biany 在 cpm 聊天里选到 security 的 docx.
    """
    meta_scope = func.json_extract(DocumentModel.meta, '$.scope')
    meta_slug = func.json_extract(DocumentModel.meta, '$.project_slug')
    return query.filter(
        or_(
            meta_scope == 'org',
            and_(meta_scope == 'personal', DocumentModel.user_id == user_id),
            and_(meta_scope == 'project', meta_slug == project_id),
        )
    )
        ...
        try:
            with get_db_context(db) as db:
                ...
                if filter:
                    ...
                    # TeleAI patch 2026-04-20: # 下拉跨项目隔离. 在 access 过滤之后再收一层 scope.
                    project_id = filter.get('project_id')
                    if project_id:
                        query = _project_scope_filter(query, Knowledge, project_id, user_id)
    ...
    def search_knowledge_files(
        ...
        try:
            with get_db_context(db) as db:
                ...
                # TeleAI patch 2026-04-20: # 下拉跨项目隔离 (同 search_knowledge_bases).
                # 这里拿不到 user_id 入参, 从 filter 里取 (router 会塞进来).
                if filter:
                    project_id = filter.get('project_id')
                    if project_id:
                        owner_id_for_personal = filter.get('user_id', '')
                        query = _project_scope_filter(query, Knowledge, project_id, owner_id_for_personal)

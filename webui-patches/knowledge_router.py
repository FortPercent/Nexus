...
async def search_knowledge_bases(
    ...
    # TeleAI patch 2026-04-20: # 下拉跨项目隔离. 前端在 letta-<slug> 聊天里传入
    # project_id=<slug>, search 会按 meta.scope 收敛: org/个人永远返, project 只返匹配.
    # 不传或传空 → 维持旧全量行为, 向后兼容.
...
async def search_knowledge_files(
    query: Optional[str] = None,
    # TeleAI patch 2026-04-20: 同 /search, 让 # 下拉的 file 搜也能按 project 收敛.
    project_id: Optional[str] = None,
    page: Optional[int] = 1,
    user=Depends(get_verified_user),
    db: Session = Depends(get_session),

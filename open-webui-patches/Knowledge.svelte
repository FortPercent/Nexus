<script lang="ts">
	...
	// letta 文件索引状态 map：letta_file_id → { processing_status, total_chunks, chunks_embedded }
	let statusMap: Record<string, any> = {};
	let statusPollTimer: ReturnType<typeof setInterval> | null = null;

	const fetchStatuses = async () => {
		try {
			const res = await fetch('/admin/api/file-statuses', {
	...
	const getMirrorStatus = (item: any) => {
		const desc = item?.description ?? '';
		if (!desc.startsWith('letta-mirror:')) return null;
		const lettaId = desc.substring('letta-mirror:'.length);
		return statusMap[lettaId] ?? null;
	};

	const hasPendingStatus = () => {
		return (items ?? []).some((it) => {
			const s = getMirrorStatus(it);
			return s && s.processing_status && s.processing_status !== 'completed' && s.processing_status !== 'error';
		});
	};
	...
	const getItemsPage = async () => {
		...
		itemsLoading = false;
		// items 变化后刷一次状态，保证新加载的 letta-mirror 行也有徽章
		fetchStatuses();
		return res;
	};
		...
		{#if items !== null && total !== null}
			{#if (items ?? []).length !== 0}
				...
				<div class=" my-2 px-3 grid grid-cols-1 lg:grid-cols-2 gap-2">
					{#each items as item}
						{@const mirrorStatus = getMirrorStatus(item)}
						<button
									...
									<div class="flex items-center justify-between -my-1 h-8">
										<div class=" flex gap-2 items-center justify-between w-full">
											<div class="flex gap-1 items-center">
												...
												{#if mirrorStatus}
													{#if mirrorStatus.processing_status === 'error'}
														<Badge type="error" content={$i18n.t('索引失败')} />
													{:else if mirrorStatus.processing_status && mirrorStatus.processing_status !== 'completed'}
														<Badge
															type="warning"
															content={mirrorStatus.total_chunks
																? `${$i18n.t('索引中')} ${mirrorStatus.chunks_embedded ?? 0}/${mirrorStatus.total_chunks}`
																: $i18n.t('索引中...')}
														/>
													{/if}
												{/if}

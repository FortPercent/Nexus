    ...
    async def grep_files(
        ...
        async def _search_files():
            ...
            for file_agent in file_agents:
                ...
                # TeleAI patch 2026-04-20: Letta 允许 FileMetadata 存在但 content=None
                # (PDF/PPT OCR / markitdown 转换失败), 原代码 file.content.encode() 直接
                # NoneType crash. skip 不致命, 明确 warning 可诊断.
                if file.content is None:
                    files_skipped += 1
                    self.logger.warning(f"Grep: Skipping file {file.file_name} - content is None (upstream OCR/convert failed)")
                    continue

...
class LettaAgentV3(LettaAgentV2):
    ...
    * Support Gemini / OpenAI client
    ...
    async def _step(
        ...
        # TeleAI patch (2026-04-21): pre-step compact for agents loaded overfull.
        # 上游只在 POST-step (line ~1439) 用 llm_adapter.usage.total_tokens 判 compact,
        # 该值要 LLM 成功返回才有. 对已超 ctx_window 的 agent, LLM 请求本身 400, estimate
        # 永远不会被设置, POST-step compact 也永远不触发, messages 无限增长. 这里在 LLM 调
        # 用前用 count_tokens_with_tools 预估, 超阈值就强制 compact.
        if self.context_token_estimate is None:
            try:
                from letta.services.summarizer.summarizer_sliding_window import count_tokens_with_tools
                ...
                self.logger.info(f"[teleai-patch] pre-step token estimate: {_teleai_estimate}")
            except Exception as _teleai_e:
                self.logger.warning(f"[teleai-patch] pre-step estimate failed: {_teleai_e}")

        if (self.context_token_estimate is not None
                and self.context_token_estimate > compaction_trigger_threshold
                and not self.agent_state.message_buffer_autoclear):
            self.logger.warning(
                f"[teleai-patch] pre-step proactive compact: {self.context_token_estimate} > {compaction_trigger_threshold}"
            )
            try:
                ...
                _teleai_summary_msg, messages, _teleai_summary_text = await self.compact(
                    ...
                    run_id=run_id,
                    step_id=_teleai_pre_step_id,
                    use_summary_role=include_compaction_messages,
                    trigger="pre_step_overfull",
                    context_tokens_before=_teleai_ctx_before,
                    messages_count_before=_teleai_mc_before,
                    billing_context=billing_context,
                )
                ...
                self.response_messages.append(_teleai_summary_msg)
                await self._checkpoint_messages(
                    run_id=run_id,
                    step_id=_teleai_pre_step_id,
                    new_messages=[_teleai_summary_msg],
                    in_context_messages=messages,
                )
                self.logger.info(
                    f"[teleai-patch] pre-step compact ok: {_teleai_mc_before} msgs → {len(messages)} msgs, est={self.context_token_estimate}"
                )
                ...
                self.logger.error(f"[teleai-patch] pre-step compact failed: {_teleai_e}; continuing anyway")

...
class LettaCoreToolExecutor(ToolExecutor):
    ...
    async def core_memory_append(self, agent_state: AgentState, actor: User, label: str, content: str) -> str:
        ...
        # TeleAI patch (2026-04-17): 行级去重——LLM 多次对话可能重复建议同一事实，
        # 原实现盲追加导致 "用户叫 X\n用户叫 X\n..."，这里按剥空白的行比对跳过重复。
        new_content_stripped = str(content).strip()
        if new_content_stripped:
            existing_lines = {ln.strip() for ln in current_value.split("\n") if ln.strip()}
            if new_content_stripped in existing_lines:
